...
def train_model(model_name, og_model, dataset, target_names, epochs=5, batch_size=128, lr=1e-3, k_folds=5, seed=0):
    """
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset)):
        ...
        num_labels = dataset[train_ids][1].sum(dim=1)
        n_samples = 0
        samples_weight = torch.zeros(num_labels.shape[0])
        for index in range(num_labels.shape[-1]):
            ...
            samples_weight += torch.tensor([weight[int(t)] for t in target])

        sampler = torch.utils.data.WeightedRandomSampler(samples_weight, len(samples_weight)) # int(max(class_sample_count) * 2)
